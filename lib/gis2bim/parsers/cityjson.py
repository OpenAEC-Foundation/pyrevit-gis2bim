...
class CityJSONParser(object):
    ...
    def _load_json_dotnet(self, filepath, file_size):
        """Laad JSON met .NET's JavaScriptSerializer (geheugen-efficienter).

        Args:
            filepath: Pad naar JSON bestand
            file_size: Bestandsgrootte in bytes (voor logging)

        Returns:
            Parsed JSON data (dict-achtig .NET object)
        ...
        try:
            ...
            serializer.MaxJsonLength = 2147483647  # int.MaxValue
            return serializer.DeserializeObject(text)

        except ImportError:
            raise CityJSONError(
                "CityJSON bestand is te groot ({0:.0f} MB) voor "
                "IronPython's JSON parser.\n"
                "Tip: gebruik 'Alleen gebouwen' i.p.v. "
                "'Gebouwen + Terreinen'.".format(size_mb))
        except Exception as e:
            if "OutOfMemory" in str(e) or "MemoryError" in str(type(e)):
                raise CityJSONError(
                    ...
                    "'Gebouwen + Terreinen'.".format(size_mb))
            raise CityJSONError(
                "Fout bij laden groot JSON bestand: {0}".format(e))
