# -*- coding: utf-8 -*-
"""
Luchtfoto Tijdreis - GIS2BIM
==============================

Download luchtfoto's van verschillende jaren via PDOK WMS
en plaats deze op een Revit sheet in een 3x2 grid.
"""
...
log, LOG_FILE = create_tool_logger("LuchtfotoTijdreis", __file__)
...
try:
    from gis2bim.api.wms import WMSClient
...
# Beschikbare luchtfoto lagen
LUCHTFOTO_LAYERS = [
    {"key": "2016_ortho25", "name": "2016 (25cm)", "layer": "2016_ortho25"},
    {"key": "2017_ortho25", "name": "2017 (25cm)", "layer": "2017_ortho25"},
    {"key": "2018_ortho25", "name": "2018 (25cm)", "layer": "2018_ortho25"},
    {"key": "2019_ortho25", "name": "2019 (25cm)", "layer": "2019_ortho25"},
    {"key": "2020_ortho25", "name": "2020 (25cm)", "layer": "2020_ortho25"},
    {"key": "2021_orthoHR", "name": "2021 (HR)", "layer": "2021_orthoHR"},
    {"key": "2022_ortho25", "name": "2022 (25cm)", "layer": "2022_ortho25"},
    {"key": "2022_orthoHR", "name": "2022 (HR)", "layer": "2022_orthoHR"},
    {"key": "2023_ortho25", "name": "2023 (25cm)", "layer": "2023_ortho25"},
    {"key": "2024_quickorthoHR", "name": "2024 (quickHR)", "layer": "2024_quickorthoHR"},
    {"key": "Actueel_ortho25", "name": "Actueel (25cm)", "layer": "Actueel_ortho25"},
    {"key": "Actueel_orthoHR", "name": "Actueel (HR)", "layer": "Actueel_orthoHR"},
    {"key": "geen", "name": "- Geen -", "layer": None},
]

# Default selecties per slot
DEFAULT_SLOTS = [
    "2016_ortho25",
    "2018_ortho25",
    "2020_ortho25",
...
def _build_layer_dict(layer_name):
    """Bouw een WMS layer dict voor de WMSClient."""
    return {
        "key": layer_name,
        "name": layer_name,
        "category": "Luchtfoto",
        "view_name": "gis2bim_luchtfoto_{0}".format(layer_name),
        "base_url": PDOK_WMS_URL,
        "layers": layer_name,
        "styles": "",
        "width": 2000,
        "height": 2000,
...
class LuchtfotoTijdreisWindow(Window):
    """WPF Window voor Luchtfoto Tijdreis."""
    ...
    def __init__(self, doc):
        ...
        bind_ui_elements(self, root, [
            ...
            'pnl_no_location',
            'cmb_bbox_size',
            'cmb_slot_1', 'cmb_slot_2', 'cmb_slot_3',
            'cmb_slot_4', 'cmb_slot_5', 'cmb_slot_6',
        ...
        self.slot_combos = [
            self.cmb_slot_1, self.cmb_slot_2, self.cmb_slot_3,
            self.cmb_slot_4, self.cmb_slot_5, self.cmb_slot_6,
        ]
        ...
        populate_sheets_dropdown(self.cmb_existing_sheet, doc,
                                 default_sheet_number="091", log=log)
        self._bind_events()

    def _populate_slot_combos(self):
        """Vul de 6 slot ComboBoxes met beschikbare luchtfoto lagen."""
    ...
    def _on_execute(self, sender, args):
        """Voer de luchtfoto tijdreis uit."""
        log("Execute gestart")
        ...
        try:
            # Haal geselecteerde lagen op
            selected_slots = self._get_selected_slots()
            active_slots = [(idx, s) for idx, s in enumerate(selected_slots) if s is not None]

            if not active_slots:
                self.txt_status.Text = "Selecteer minimaal 1 luchtfoto"
                return
            ...
            for slot_idx, layer_key in active_slots:
                layer_dict = _build_layer_dict(layer_key)
                show_progress(self, "Downloaden: {0}...".format(layer_key))

                try:
                    image_path = client.download_image(layer_dict, bbox)
                    downloaded_images.append((slot_idx, layer_key, image_path))
                    log("Download OK: {0} -> {1}".format(layer_key, image_path))
                except Exception as e:
                    errors.append("{0}: download fout - {1}".format(layer_key, str(e)))
                    log("Download fout {0}: {1}".format(layer_key, e))

            if not downloaded_images:
                self.txt_status.Text = "Geen images gedownload. Controleer de verbinding."
                hide_progress(self)
            ...
            t = DB.Transaction(self.doc, "GIS2BIM - Luchtfoto Tijdreis")
            t.Start()
            ...
            try:
                ...
                placed_count = 0
                for slot_idx, layer_key, image_path in downloaded_images:
                    show_progress(self, "Plaatsen: {0}...".format(layer_key))
                    ...
                    try:
                        ...
                        place_label_on_sheet(
                            self.doc, sheet, layer_key,
                            center_x, center_y, layout["img_size"],
                            label_offset_mm=LABEL_OFFSET_MM, log=log
                        )
                        placed_count += 1
                        log("Geplaatst: {0} op positie {1}".format(layer_key, slot_idx + 1))
                    except Exception as e:
                        errors.append("{0}: plaatsingsfout - {1}".format(layer_key, str(e)))
                        log("Plaatsingsfout {0}: {1}".format(layer_key, e))
                        log(traceback.format_exc())
    ...
    def _get_selected_slots(self):
        """Haal geselecteerde layer keys op per slot. None = 'geen'."""
        result = []
    ...
    def _build_centered_layout(self, sheet):
        ...
        Gebruikt de door de gebruiker opgegeven beschikbare ruimte
        en centreert het grid op het midden van de sheet (titelblok).
    ...
    def _show_result(self, placed_count, errors):
        ...
        msg_lines = [
            "Luchtfoto tijdreis gereed!",
            "",
            "Foto's geplaatst: {0}".format(placed_count),
        ]
        ...
        forms.alert("\n".join(msg_lines), title="GIS2BIM - Luchtfoto Tijdreis")


def main():
    clear_log_file(LOG_FILE)
    log("=== GIS2BIM Luchtfoto Tijdreis Tool gestart ===")
    ...
    try:
        window = LuchtfotoTijdreisWindow(doc)
        window.ShowDialog()
    except Exception as e:
        log("Window error: {0}\n{1}".format(e, traceback.format_exc()))
        forms.alert(
    ...
    log("=== GIS2BIM Luchtfoto Tijdreis Tool beeindigd ===")
