...
- [ ] **D7 — curtain-walls/vliesgevels zonder SEGC sub-boundary → 0-vertex `fallback`-construction (géén geometrie in export) → 3D-viewer in open-heatloss-studio kan ze niet tekenen.** Geverifieerd 15-06 op model 2786: con-147..151 op room-14/16/19/20, `revit_type_name` eindigt op " fallback" (`28_CWA_glas15mm fallback`, `Opening fallback`); gekoppelde openings (open-23/24/27/28/29, type `curtain_wall`) óók 0 vertices. Curtain-walls zijn doorgaans niet Room Bounding → SEGC `sub_faces.Count==0` → scanner skipt de wand (memory-valkuil: walls zonder sub-boundary bewust geskipt). Fix: curtain-walls/vliesgevels face-based geometrie geven (parent-face + ReferenceIntersector, zoals de horizontale-face-fix `39c6768`) i.p.v. skippen. **Vereist Revit + model open voor test.** (`raycast_scanner.py` SEGC-wandpad)
