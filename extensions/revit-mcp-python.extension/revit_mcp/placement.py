...
def register_placement_routes(api):
    ...
    @api.route("/place_workplane_families/", methods=["POST"])
    def place_workplane_families(request):
        """
        Place WorkPlaneBased families on vertical planes (for windows, doors, etc).
        
        Expected data:
        {
            "symbol_id": 1281171,  (FamilySymbol ElementId)
            "placements": [
                {
                    "x_mm": 4335.8,
                    "y_mm": 43672.5,
        ...
        try:
            ...
            if not symbol_id:
                return routes.make_response(data={"error": "symbol_id required"}, status=400)
            ...
            if not symbol:
                return routes.make_response(
                    data={"error": "Symbol with ID {} not found".format(symbol_id)},
                    status=404
                )
            
            results = {"placed": [], "failed": []}
            
            t = DB.Transaction(doc, "MCP Place WorkPlane Families")
            t.Start()
            ...
            try:
                ...
                for i, p in enumerate(placements):
                    try:
                        # Get coordinates (convert mm to feet)
                        ...
                        rotate_180 = p.get("rotate_180", 0)
                        mark = p.get("mark", "")
                        
                        point = DB.XYZ(x_ft, y_ft, z_ft)
                        normal = DB.XYZ(nx, ny, nz)
                        
                        # Create vertical sketch plane
                        ...
                        if rotate_180 == 1:
                            axis = DB.Line.CreateBound(
                                point,
                                DB.XYZ(point.X + normal.X, point.Y + normal.Y, point.Z + normal.Z)
                            )
            ...
            logger.error("Place workplane families failed: {}".format(str(e)))
            return routes.make_response(data={"error": str(e)}, status=500)

    @api.route("/list_families/", methods=["GET"])
    def list_families():
        """Get a list of family names and types with correct family names."""
        doc = revit.doc
        try:
            if not doc:
                return routes.make_response(
            ...
            for symbol in symbols:
                if len(families) >= 100:
                    break
                try:
                    # Get CORRECT family name via FamilyName property (IronPython compatible)
                    fam_name = symbol.FamilyName if hasattr(symbol, 'FamilyName') else "Unknown"
                    # Get type name via Element.Name getter (IronPython compatible)
                    type_name = DB.Element.Name.__get__(symbol) if symbol else "Unknown"
