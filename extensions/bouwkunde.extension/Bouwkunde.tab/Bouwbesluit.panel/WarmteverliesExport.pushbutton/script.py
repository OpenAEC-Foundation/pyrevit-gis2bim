...
from warmteverlies.uvalue_extractor import get_u_value, extract_layers
from warmteverlies.json_builder import build_project_json, export_to_file
from warmteverlies.constants import DEFAULT_ROOM_HEIGHT, DEFAULT_U_VALUES
...
def _get_type_name_safe(element):
    """Haal de type naam van een element, veilig voor curtain walls."""
    try:
        elem_type = element.Document.GetElement(element.GetTypeId())
        if elem_type:
            return elem_type.FamilyName or "Curtain Wall"
    except Exception:
        pass
    return "Curtain Wall"
...
def run_export(doc):
    ...
    for room_data in heated_rooms:
        ...
        for boundary in boundaries:
            ...
            host_doc = boundary.get("host_doc", doc)
            u_val, u_src = get_u_value(
                doc, host, pos, bt, element_doc=host_doc
            )
            boundary["u_value"] = u_val
            boundary["u_source"] = u_src

            # Laagopbouw extractie
            layers = extract_layers(doc, host, element_doc=host_doc)
            boundary["layers"] = layers

        # Openings per wall
        openings_by_wall = {}
        curtain_wall_ids = set()
        for boundary in boundaries:
            host = boundary.get("host_element")
            ...
            if host_cat == "Wall" and host is not None:
                # Check of de wand zelf een curtain wall is
                try:
                    is_curtain = (
                        host.CurtainGrid is not None
                    )
                except Exception:
                    is_curtain = False

                if is_curtain:
                    # Hele wand is curtain wall: behandel als opening
                    cw_opening = {
                        "element": host,
                        "element_id": host_id,
                        ...
                        "u_value": DEFAULT_U_VALUES["window"],
                        "name": _get_type_name_safe(host),
                    }
                    openings_by_wall.setdefault(
                        host_id, []
                    ).append(cw_opening)
                    curtain_wall_ids.add(host_id)
                    continue

                host_doc = boundary.get("host_doc", doc)
                wall_openings = extract_openings(
                    doc, host, element_doc=host_doc
                )
                if wall_openings:
