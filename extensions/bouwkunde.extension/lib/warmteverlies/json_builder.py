...
def _build_construction(
    ...
    # Laagopbouw uit eerste boundary in de groep
    # (alle boundaries in een groep hebben hetzelfde type, dus
    # dezelfde laagopbouw)
...
def _opening_description(category, openings):
    ...
    elif category == "curtain_wall":
        if count == 1:
            return "Vliesgevel"
        return "Vliesgevels ({0} stuks)".format(count)
