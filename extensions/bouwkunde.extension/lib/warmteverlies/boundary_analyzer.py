...
from warmteverlies.link_resolver import resolve_boundary_element
...
def _analyze_3d(doc, room):
    ...
    for face in spatial_solid.Faces:
        ...
        for sub_face in sub_faces:
            ...
            resolved = resolve_boundary_element(doc, sub_face)
            ...
            boundaries.append({
                "area_m2": area_m2,
                "host_element": resolved["host_element"],
                "host_element_id": resolved["host_element_id"],
                "host_category": resolved["host_category"],
                "host_doc": resolved["host_doc"],
                "link_instance_id": resolved["link_instance_id"],
                "face_normal": (normal.X, normal.Y, normal.Z),
                "position_type": position_type,
            })

    # Synthetische ceiling als SEGC geen bovenvlak retourneert
    # (bijv. dak niet room-bounding)
    floor_area_m2 = internal_to_sqm(room.Area)
    if not has_ceiling and floor_area_m2 >= MIN_FACE_AREA_M2:
        boundaries.append({
    ...
    # Synthetische vloer als SEGC geen ondervlak retourneert
    if not has_floor and floor_area_m2 >= MIN_FACE_AREA_M2:
        boundaries.append({
            "area_m2": floor_area_m2,
            "host_element": None,
            "host_element_id": None,
                ...
                if host_element:
                    ...
                    if cat is not None:
                        ...
                        if cat_id == -2000011:
                            ...
                            host_category = "Ceiling"
                        else:
                            host_category = cat.Name
