...
# Patronen voor ruimten die buitenruimten zijn (niet verwarmd, geen
# onderdeel van de thermische schil)
OUTSIDE_PATTERNS = [
    re.compile(r"buiten\b", re.IGNORECASE),
    re.compile(r"terras", re.IGNORECASE),
    re.compile(r"balkon", re.IGNORECASE),
    re.compile(r"^room$", re.IGNORECASE),    # Revit placeholder
...
def is_likely_outside(room_name):
    """Controleer of een ruimte waarschijnlijk een buitenruimte is.

    Buitenruimten (terras, balkon, etc.) zijn per definitie niet verwarmd
    en horen niet in de warmteverliesberekening.

    Args:
        room_name: Ruimtenaam als string

    Returns:
        bool: True als de ruimte waarschijnlijk een buitenruimte is
    """
    if not room_name:
        return False
