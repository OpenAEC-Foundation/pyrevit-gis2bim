...
def build_element_room_lookup(doc, all_rooms):
    """Bouw lookup: element_id -> set(room_element_ids).

    Publieke wrapper rond _build_element_room_lookup voor gebruik
    door supplementaire element scan in WarmteverliesView.

    Args:
        doc: Revit Document
        all_rooms: Alle room data dicts (uit room_collector)

    Returns:
        dict: {element_id_int: set(room_element_id_ints)}
    """
    return _build_element_room_lookup(doc, all_rooms)
