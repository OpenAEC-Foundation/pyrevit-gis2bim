...
def extract_openings(doc, wall, element_doc=None):
    """Extraheer alle ramen en deuren uit een wand.

    Args:
        doc: Revit Document (host document)
        wall: Revit Wall element
        element_doc: Document waarin de wand leeft (linked model of host).
    ...
    for insert_id in insert_ids:
        element = element_doc.GetElement(insert_id)
        if element is None:
            continue
...
def _classify_opening(element):
    """Classificeer een element als window, door of curtain_wall."""
