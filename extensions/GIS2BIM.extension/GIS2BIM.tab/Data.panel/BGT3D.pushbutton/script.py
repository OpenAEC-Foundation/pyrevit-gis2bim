...
try:
    ...
    from gis2bim.revit.families import (
...
# Map met meegeleverde .rfa bestanden
FAMILIES_DIR = os.path.join(os.path.dirname(__file__), "families")

# Tag voor de automatische laad/fallback optie
AUTO_TAG = "__AUTO__"
...
def _extract_points_from_features(features):
    """Extraheer unieke (x, y) punten uit OGCAPIFeature objecten.

    Ondersteunt point, multipoint en polygon geometrie (centroid).
    Dedupliceert op afgeronde coordinaten (mm-precisie) om
    historische versies van hetzelfde BGT object te filteren.

    Returns:
        Lijst van unieke (x, y) tuples
    """
    points = []
    seen = set()
    ...
    # Deduplicatie: rond af op mm om floating-point ruis te negeren
    unique = []
    for pt in points:
...
class BGT3DWindow(Window):
    ...
    def _populate_family_dropdowns(self):
        """Vul family dropdowns met auto-optie en alle families uit het document."""
        ...
        for combo, default_name in [
            ...
            # Auto-optie als eerste item
            auto_item = ComboBoxItem()
            auto_item.Content = "[Automatisch laden / generiek]"
            auto_item.Tag = AUTO_TAG
            combo.Items.Add(auto_item)
            ...
            for name in family_names:
                ...
                if name == default_name:
                    # +1 vanwege auto-item op index 0
                    default_index = combo.Items.Count - 1

            if default_index >= 0:
                combo.SelectedIndex = default_index
                log("Family '{0}' voorgeselecteerd".format(default_name))
            else:
                # Default family niet gevonden → selecteer auto-optie
                combo.SelectedIndex = 0
                log("Family '{0}' niet gevonden, auto-optie geselecteerd".format(
                    default_name))
        ...
        if warnings:
            self.txt_family_warning.Text = (
                "Standaard families niet gevonden: {0}. "
                "De auto-optie laadt .rfa bestanden of genereert 3D geometrie."
            ).format(", ".join(warnings))
        else:
            self.txt_family_warning.Text = ""

    def _get_selected_tag(self, combo):
        """Haal de Tag op van het geselecteerde ComboBox item.

        Returns:
            Tag string (family naam of AUTO_TAG), of None
        """
    ...
    def _resolve_strategy(self, combo, default_name, fallback_type):
        """Bepaal PlacementStrategy op basis van combobox selectie.

        Args:
            combo: ComboBox met family selectie
            default_name: Default family naam
            fallback_type: "tree" of "lamp" voor DirectShape

        Returns:
            PlacementStrategy of None bij ongeldige selectie
        """
        tag = self._get_selected_tag(combo)
        if tag is None:
            return None

        if tag == AUTO_TAG:
            return resolve_family_or_fallback(
                self.doc, default_name, FAMILIES_DIR, fallback_type)

        # Specifieke family geselecteerd
        symbol = self._family_map.get(tag)
        if symbol is None:
            return None

        return PlacementStrategy(
            mode="family", family_symbol=symbol, family_name=tag)
    ...
    def _on_execute(self, sender, args):
        ...
        # Bepaal placement strategies
        strategy_bomen = None
        strategy_lantaarns = None

        if plaats_bomen:
            strategy_bomen = self._resolve_strategy(
                self.cmb_family_bomen, DEFAULT_FAMILY_BOMEN, "tree")
            if strategy_bomen is None:
                self.txt_status.Text = "Selecteer een family voor bomen"
                return

        if plaats_lantaarns:
            strategy_lantaarns = self._resolve_strategy(
                self.cmb_family_lantaarns, DEFAULT_FAMILY_LANTAARNS, "lamp")
            if strategy_lantaarns is None:
        ...
        try:
            ...
            log("Center RD: x={0}, y={1} - Zoekstraal: {2}m".format(
                center_x, center_y, search_radius))
            log("BBox: xmin={0}, ymin={1}, xmax={2}, ymax={3}".format(
                bbox[0], bbox[1], bbox[2], bbox[3]))

            # Validatie: RD-coördinaten moeten in bereik liggen
            if not (10000 < center_x < 280000 and 300000 < center_y < 625000):
                log("WAARSCHUWING: Coördinaten lijken niet in RD bereik te liggen!")
                log("  Verwacht: x=10000-280000, y=300000-625000")
            ...
            if plaats_bomen:
                ...
                log("Bomen strategy: {0}".format(strategy_bomen))

                if bomen_points:
                    show_progress(self, "Bomen plaatsen ({0} stuks)...".format(
                        len(bomen_points)))
                    update_ui()
                    bomen_count = self._place_elements(
                        bomen_points, strategy_bomen,
            ...
            if plaats_lantaarns:
                ...
                log("Lantaarns strategy: {0}".format(strategy_lantaarns))

                if lantaarn_points:
                    show_progress(self, "Lantaarnpalen plaatsen ({0} stuks)...".format(
                        len(lantaarn_points)))
                    update_ui()
                    lantaarns_count = self._place_elements(
                        lantaarn_points, strategy_lantaarns,
            ...
            self._show_result(
                search_radius, bomen_count, lantaarns_count,
                strategy_bomen, strategy_lantaarns)
    ...
    def _place_elements(self, points, strategy, origin_x, origin_y, label):
        """Plaats elementen op de gegeven punten via PlacementStrategy.
        ...
        try:
            ...
            for i, pt in enumerate(points):
                try:
                    ...
                    name = "{0} {1}".format(label, i + 1)
                    element = place_element(self.doc, strategy, xyz, name=name)
            ...
            t.Commit()
            log("{0}: {1} geplaatst, {2} fouten (modus: {3})".format(
                label, placed, errors, strategy.mode))
            return placed
    ...
    def _show_result(self, search_radius, bomen_count, lantaarns_count,
                     strategy_bomen=None, strategy_lantaarns=None):
        """Toon resultaat dialog met informatie over gebruikte modus."""
        msg_lines = [
        ...
        if bomen_count > 0:
            modus = self._strategy_label(strategy_bomen)
            msg_lines.append("Bomen geplaatst: {0} ({1})".format(
                bomen_count, modus))
        if lantaarns_count > 0:
            modus = self._strategy_label(strategy_lantaarns)
            msg_lines.append("Lantaarnpalen geplaatst: {0} ({1})".format(
                lantaarns_count, modus))
    ...
    def _strategy_label(strategy):
        """Geeft een leesbare omschrijving van de gebruikte strategie.

        Args:
            strategy: PlacementStrategy object of None

        Returns:
            String met modus omschrijving
        """
        if strategy is None:
            return "niet geplaatst"
        if strategy.mode == "family":
            return "family: {0}".format(strategy.family_name)
        return "generieke 3D geometrie"
